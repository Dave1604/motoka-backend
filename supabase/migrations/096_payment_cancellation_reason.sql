-- Migration 096: RECOVERED copy of _duplicates/064_payment_cancellation_reason.sql.
-- The 064-067 renumbering moved it to _duplicates/ without a successor, so no
-- migration in the sequence created a column live code writes to. Production
-- already has it (applied by hand); every statement below is IF NOT EXISTS-safe.

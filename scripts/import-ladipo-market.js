...
function pickBestImages(incomingUrl, existingImages = []) {
  const incoming = toHttps(incomingUrl);
  if (isUsableImage(incoming)) return [incoming];
  const existing = Array.isArray(existingImages)
    ? existingImages.map(toHttps).find(isUsableImage)
    : null;
...
const CATEGORY_SLUG_MAP = {
  ...
  // Mechanical gearboxes / gears — NOT Gear Oil & ATF (fluids).
...
function extractProductsFromCategoryPage(html, categorySlug) {
  ...
  while ((match = blockRegex.exec(html)) !== null) {
    ...
    products.push({
      ...
      title,
      slug,
      sku: `LM-${slug.toUpperCase().slice(0, 20)}-${products.length + 1}`,

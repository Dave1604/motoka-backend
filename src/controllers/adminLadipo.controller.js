...
const MAX_PRICE_KOBO = 100_000_000_00; // ₦1 billion max

function normalizeProductPayload(payload = {}) {
  const slug = String(payload.slug || '').trim().toLowerCase().slice(0, 200);
  const name = String(payload.name || '').trim().slice(0, 255);
  const categoryId = payload.category_id || null;
  const brand = payload.brand ? String(payload.brand).trim().slice(0, 100) : null;
  const description = payload.description ? String(payload.description).trim().slice(0, 4000) : null;
  ...
  if (!SLUG_RE.test(slug)) {
    throw new Error('slug must be lowercase alphanumeric with hyphens only (e.g. brake-pad-toyota)');
  }
  if (!categoryId) {
  ...
  if (price_kobo > MAX_PRICE_KOBO) {
    throw new Error('price_kobo exceeds the maximum allowed value');
  }
  if (!Number.isInteger(stock_qty) || stock_qty < 0) {
    throw new Error('stock_qty must be a non-negative integer');
  }
  if (stock_qty > 1_000_000) {
    throw new Error('stock_qty exceeds the maximum allowed value');
  }
...
export async function listLadipoOrders(req, res) {
  try {
    ...
    const statusFilter = String(req.query.status || 'all').trim().toLowerCase().slice(0, 40);
    const dbStatusFilter = LADIPO_ADMIN_STATUS_MAP[statusFilter] || null;
    const workflowState = String(req.query.workflow_state || '').trim().toLowerCase().slice(0, 20);
    const unassignedOnly = String(req.query.unassigned || '').toLowerCase() === '1'
      || String(req.query.unassigned || '').toLowerCase() === 'true';
    const handlerKey = String(req.query.handler_key || '').trim().toLowerCase().slice(0, 120);
